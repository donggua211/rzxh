...
<div id="main">
	<div id="main_body">
		...
		<form action="<?php echo site_url('admin/group/add') ?>" method="post" name="addstaff">
			...
			<tr>
				...
				<td>
					...
					foreach($rooms as $room)
						echo '<tr><td style="border-bottom:1px"><input name="room['.$room['room_id'].']" type="checkbox" value="'.$room['room_id'].'" />'.$room['room_name'].'</td><td style="border-bottom:1px"><input name="role['.$room['room_id'].']" type="radio" value="'.GROUP_ROLE_READABLE.'" />可读</td><td><input name="role['.$room['room_id'].']" type="radio" value="'.GROUP_ROLE_CONFIGABLE.'" />可读可配置</td></tr>';
					?>
					</table>
				</td>
			</tr>

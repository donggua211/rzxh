...
class client_socket{
	 ...
	 * @var string
	 */
	private $sBuffer_subfix = " /\r\n";

	/**
	 * Read TimeOut (seconds)
	 * @var int
	 */
	private $iReadTimeOut = 8;
	...
	public function error($msg=null){
		...
		if($errCode!=0){
			...
			if($this->bExceptions)
				show_error("Socket error. Code: $errCode - Message: $errMsg");
			else
				show_error("Socket Error. Code: $errCode - Message: $errMsg");
			socket_clear_error($this->hnd);
		}elseif (strlen($msg)){
			if($this->bExceptions){
				show_error("Socket error." . $msg);
			}
			else{
				show_error("$msg\n",E_USER_ERROR);
			}
	...
	public function send($sBuf,$iTimeOut=null){
		...
		$tmpBuf		= ( strlen($sBuf) ? $sBuf : $this->sBuffer ) . $this->sBuffer_subfix;
	...
	public function recv($iTimeOut=null){
		...
		//处理结果： 转码，解析
		return xml_to_array(iconv("GB2312", "UTF-8", $this->sBuffer));
	}
